...
func (flt *GogsHook) Filter(triggerParam, filterParam map[string]interface{}) (map[string]interface{}, bool) {
	...
	if gogsEvent != "" && gogsEvent != event {
		flt.Debugf(`Event does not match: "%s"`, event)
		return nil, false
	}
	...
	if err != nil {
		flt.Errorf(`Invalid clone url: "%s"`, cloneURL)
		return nil, false
	}
	...
	if gogsHost != "" && gogsHost != host {
		flt.Debugf(`Host does not match: "%s"`, host)
		return nil, false
	}

	if gogsBranch != "" && gogsBranch != branch {
		flt.Debugf(`Branch does not match: "%s"`, branch)
		return nil, false
	}
	...
	if gogsName != "" {
		if strings.ContainsAny(gogsName, "/") {
			if gogsName != repoFullName {
				flt.Debugf(`Name does not match: "%s"`, repoFullName)
				return nil, false
			}
		} else {
			if gogsName != repoName {
				flt.Debugf(`Full name does not match: "%s"`, repoName)
				return nil, false
			}
		}
	}
